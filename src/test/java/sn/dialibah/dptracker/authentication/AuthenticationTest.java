package sn.dialibah.dptracker.authentication;
...
import sn.dialibah.dptracker.ApplicationTest;
...
public class AuthenticationTest {
	...
	public void testBothPasswords() throws InvalidPasswordException {
		...
		SignupDataBean signupDataBean = SignupDataBean.builder().login("osow").firstName("Ousmane SOW").confirmPassword("test").build();
		Profile retrievedProfile = this.authenticationService.signup(signupDataBean);
		Assert.assertNull(retrievedProfile);
	...
	public void testLoginMongo() {
		...
		try{
			loggedInProfile = this.authenticationService.login(LoginDataBean.builder().login("kissMongoUser").password("badPassword").build());
		...
		try{
			loggedInProfile = this.authenticationService.login(LoginDataBean.builder().login("kissMongoUser").password("mongo").build());
		}catch(UnableToLoginException le){
			exceptedException = le;
		}
