...
import sn.dialibah.dptracker.authentication.exceptions.TokenNotValidException;
import sn.dialibah.dptracker.common.configurations.security.models.TokenWrapper;
...
public class CryptoService implements ICryptoService {
	...
	@Autowired
	public CryptoService(@Value("${kiss.security.jwt-secret}") String secret) {
		try{
	...
	private DecodedJWT verifyAndDecodeToken(String token) {
		TokenEntity issuedToken = null;
		//		this.tokenRepository.findByJsonWebToken(token);
		if(issuedToken == null)
			throw new TokenNotValidException("Token revoked or never issued");
